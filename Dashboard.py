...
with col1:
    st.image("adidas-logo.jpg", height=100, width=100)

with col2:
    st.markdown(
        "<h1 style='text-align:left; font-weight:bold; color:white;'>Adidas Interactive Sales Dashboard</h1>",
        unsafe_allow_html=True
    )
...
with dwn1:
    st.download_button("Get Data",data=data.to_csv().encode("utf-8"),
                       file_name="RetialerSales.csv",mime="text/csv")
...
with dwn2:
    st.download_button("Get Data",data = result.to_csv().encode("utf-8"),
                       file_name="Monthly Sales.csv",mime="txt/csv")
...
fig3.add_trace(go.Scatter(x=result1["State"],
                          y = result1["UnitsSold"],
                          mode = "lines",
                          name ="Units Sold",
                          yaxis="y2"
